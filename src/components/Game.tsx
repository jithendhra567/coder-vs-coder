...
import { color1, color2, debounce, Move, Moves, position, Room, Tile, TileRow, User, usersAtom } from '../utils/constant';
import '../index.css';
...
export const Game = () => {
  ...
  const allMovesMap = useRef<Map<string, Move>>(new Map()).current;
  ...
  const updateRoomData = () => {
    if (roomData.current) {
      const data = roomData.current;
      setUsers(data.users);
    }
  }
  const updatingRoomData = debounce(updateRoomData,200);
  useEffect(updatingRoomData, [roomData.current]);
  ...
  const updatingMovesData = debounce(updateMovesData,200);
  useEffect(updatingMovesData, [moves.current]);
  ...
  const updatingCompletedMovesData = debounce(updateCompletedMovesData,200);
  useEffect(updatingCompletedMovesData, [completedMoves.current]);
  ...
  const run = async () => {
    ...
    await fetchGraphQL(addMove(move, roomId), "MyMutation", {});
    // await attackFrom([18,0],[0, 18], users[0]);
  ...
  const addMove = (move: Move, roomId: string) => `
    mutation MyMutation {
