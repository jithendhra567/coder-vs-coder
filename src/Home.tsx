...
const Home = () => {
  ...
  const login = async (name: string, color: string, roomId: string, position: [number, number]) => {
    ...
    await addUserToRoom(name, color, roomId, position);
    if (errors) console.error(errors);
    else {
      history.push({ pathname: '/game', state: { roomId: roomId, userName: name, isLeader: true } });
    }
  } 

  const joinRoom = async (name: string, color: string, roomId: string, position: [number, number]) => {
    addUserToRoom(name, color, roomId, position);
    history.push({ pathname: '/game', state: { roomId: roomId, userName: name, isLeader: false } });
  }
  
  const addUserToRoom = async (name: string, color: string, roomId: string, position: [number, number]) => {
    const addUser = `
      mutation MyMutation {
        adduser(input: {id: "${name}", color: "${color}", name: "${name}", position: {i: ${position[0]}, j: ${position[1]}}, power: 100, roomInfo: {id: "${roomId}"}}) {
          numUids
        }
      }    
    `;
  ...
  const join = () => {
    ...
    if (nameDoc && roomIdDoc && isJoining.current) {
      //joining
      joinRoom(nameDoc.value, color2, roomIdDoc.value, [18, 18]);
      return;
    }
